  ...
  const generateSmallImage = async (imageSource) => {
    ...
    try {
      if (imageSource.startsWith('data:')) {
        ...
        try {
          // Add cache-busting timestamp to URL to avoid browser cache issues
          const cacheBusterUrl = `${imageSource}${imageSource.includes('?') ? '&' : '?'}t=${Date.now()}`;

          // Simple fetch usually works best if bucket CORS is set correctly
          ...
          console.warn('CORS/Fetch error on remote image, using original:', fetchErr);
          // If we can't resize it (due to CORS), we just use the large one.
