...
export default function CartClient() {
    ...
    const refreshCart = async () => {
      ...
      if (active) {
        ...
        const mergedCart = cart.map(item => {
           const update = updates.find(u => u.productId === item.productId && u.variantId === item.variantId);
  ...
  const remainingForExtraDiscount = !isFirstShopEligible ? Math.max(0, EXTRA_DISCOUNT_THRESHOLD - subtotal) : 0;
  ...
  // 4. Shipping Logic
  const payableBeforeShipping = subtotal - totalProductMarkdown - totalExtraDiscount - firstShopDiscountAmount;
    ...
    <Container maxWidth="lg" sx={{ pb: 8 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="body2" sx={{ color: '#1565C0', fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>
                Your order is over 5,000 ֏ — shipping will be free at checkout.
              </Typography>
            </Box>
          </Box>
          <Box
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                <Typography variant="body2" sx={{ color: '#2E7D32', fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>
                  You have 20% discount for this shop!
                </Typography>
              </Box>
            </Box>
            <Box
         ...
         <Box
          ...
          >
            <Typography variant="body2" sx={{ color: '#E65100', fontSize: { xs: '0.85rem', sm: '0.95rem' } }}>
               🔥 Add items worth <b>{remainingForExtraDiscount.toLocaleString()} ֏</b> more to get 10% extra discount!
            </Typography>
          </Box>
      ) : (
          <Box
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                <Typography fontWeight="bold" sx={{ color: '#1B5E20', fontSize: { xs: '0.9rem', sm: '1rem' } }}>
                  Congratulations — extra discount unlocked
                </Typography>
                <Typography variant="body2" sx={{ color: '#2E7D32', fontSize: { xs: '0.8rem', sm: '0.9rem' } }}>
                  Your order amount exceeded {EXTRA_DISCOUNT_THRESHOLD.toLocaleString()}֏ — enjoy an extra 10% at checkout.
                </Typography>
              </Box>
            </Box>
            <Box
      ...
      <Grid container spacing={4}>
        ...
        <Grid size={{ xs: 12, md: 12, lg: 8 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {cart.map((item) => {
              ...
              const finalUnitPrice = item.quantity > 0 ? (originalPrice * item.quantity - totalReductions) / item.quantity : 0;
              ...
              return (
                <Card
                  ...
                  elevation={0}
                  sx={{
                    p: { xs: 1.5, sm: 2 }, // Decreased padding for mobile
                    position: 'relative',
                    border: '1px solid',
                    borderColor: 'divider',
                  ...
                  <Grid container spacing={2} alignItems="flex-start">
                    ...
                    <Grid size="grow">
                      <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'space-between' }}>
                        <Box sx={{ pr: 4 }}> {/* Padding Right for Checkbox space */}
                          <Typography
                            variant="subtitle2"
                            fontWeight="bold"
                        ...
                        <Box sx={{ mt: 1 }}>
                            <Box sx={{ display: 'flex', alignItems: 'baseline', gap: 1 }}>
                                ...
                                {(existingDiscountPercent > 0 || itemExtraDiscount > 0 || itemFirstShopDiscount > 0) && (
                            ...
                            {markdownAmount > 0 && (
                                <Typography variant="caption" display="block" color="#E65100" fontWeight="bold">
                                    Markdown: -{markdownAmount.toLocaleString()} ֏
                                </Typography>
                            )}
                            {itemFirstShopDiscount > 0 && (
                                <Typography variant="caption" display="block" color="#E65100" fontWeight="bold">
                                    First Shop Discount: -{itemFirstShopDiscount.toLocaleString()} ֏
                                </Typography>
                            )}
                            {itemExtraDiscount > 0 && (
                                <Typography variant="caption" display="block" color="#E65100" fontWeight="bold">
                                    Extra Discount ({extraPercentApplied}%): -{itemExtraDiscount.toLocaleString()} ֏
                                </Typography>
                            )}
                        </Box>
                    ...
                    <Grid size={{ xs: 12, sm: 'auto' }} sx={{ display: { xs: 'none', sm: 'flex' }, flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'flex-end', height: '100%', minHeight: 120 }}>
        ...
        <Grid size={{ xs: 12, lg: 4 }}>
           <Box sx={{ position: { lg: 'sticky' }, top: 20 }}> {/* Sticky container */}
            <Card variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1, color: '#E65100' }}>
                    <Typography>Product Markdown</Typography>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1, color: '#E65100' }}>
                    <Typography>First Shop Discount</Typography>
                ...
                {totalSavings > 0 && (
                    <Box sx={{ mt: 2, mb: 2, p: 2, bgcolor: '#FFF3E0', borderRadius: 2, border: '1px dashed #E65100' }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1, color: '#E65100' }}>
                            <Typography fontWeight="bold">Total Savings</Typography>
                             <Typography fontWeight="bold">{totalSavings.toLocaleString()} ֏</Typography>
                        </Box>
                        {totalProductMarkdown > 0 && (
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem', color: '#E65100', opacity: 0.8 }}>
                                <Typography>• Product markdowns</Typography>
